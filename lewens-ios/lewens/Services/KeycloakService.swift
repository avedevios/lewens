...
class KeycloakService: NSObject, ObservableObject {
    ...
    // Public accessor for access token
    var currentUserToken: String? {
        if let token = authState?.lastTokenResponse?.accessToken {
            return token
        }
        
        // If no token in memory, try to restore from Keychain synchronously
        if let authStateData = KeychainHelper.shared.read(service: "lewens-ios", account: "auth_state"),
           let restoredState = try? NSKeyedUnarchiver.unarchivedObject(ofClass: OIDAuthState.self, from: authStateData) {
            self.authState = restoredState
            return restoredState.lastTokenResponse?.accessToken
        }
    ...
    private func saveAuthData() {
        ...
        if let authState = authState,
           let authStateData = try? NSKeyedArchiver.archivedData(withRootObject: authState, requiringSecureCoding: false) {
            KeychainHelper.shared.save(authStateData, service: "lewens-ios", account: "auth_state")
        }
    }
    ...
    private func clearAuthData() {
        ...
        KeychainHelper.shared.delete(service: "lewens-ios", account: "auth_state")
        // Also remove legacy UserDefaults key if it exists
